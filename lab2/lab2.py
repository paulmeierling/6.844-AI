...
def extensions(graph, path):
    ...
    for edge in graph.edges:
        new_path = path.copy()
        if edge.endNode == last_node:
            new_path.append(edge.startNode)
            if not (has_loops(new_path)): new_paths.append(new_path)

        elif edge.startNode == last_node:
...
def sort_by_heuristic(graph, goalNode, nodes):
    ...
    return [node for _,node in sorted(zip(lengths,nodes))] #Not super readable but it works
...
def basic_dfs(graph, startNode, goalNode):
    ...
    while len(agenda) >= 1:
        ...
        if current_path[-1] == goalNode: return current_path
...
def sort_paths_by_goal_distance(graph, goalNode, paths):
    """
    Orders paths by distance to goalNode
    """
    path_lengths = [graph.get_heuristic_value(p[-1], goalNode) for p in paths]
    return [path for _,path in sorted(zip(path_lengths,paths))]

def sort_paths_by_length(graph, goalNode, paths):
    """
    Orders paths by their respective length 
    """
    path_lengths = [path_length(graph,p) for p in paths]
    return [path for _,path in sorted(zip(path_lengths,paths))]

def sort_paths_by_len_and_goal(graph, goalNode, paths):
    """
    Orders paths by distance to goalNode + lengths of path
    """
    path_heuristics = [graph.get_heuristic_value(p[-1], goalNode) + path_length(graph,p) for p in paths]
    return [path for _,path in sorted(zip(path_heuristics,paths))]

generic_dfs = [do_nothing_fn, True, do_nothing_fn, False]
generic_bfs = [do_nothing_fn, False, do_nothing_fn, False]
generic_hill_climbing = [sort_paths_by_goal_distance, True, do_nothing_fn, False]
generic_best_first = [do_nothing_fn, True, sort_paths_by_goal_distance, False]
generic_branch_and_bound = [do_nothing_fn, True, sort_paths_by_length, False]
generic_branch_and_bound_with_heuristic = [do_nothing_fn, True, sort_paths_by_len_and_goal, False]
generic_branch_and_bound_with_extended_set = [do_nothing_fn, True, sort_paths_by_length, True]
generic_a_star = [do_nothing_fn, True, sort_paths_by_len_and_goal, True]

# Here is an example of how to call generic_search (uncomment to run):
#print(generic_search(*generic_bfs)(GRAPH_2, 'S', 'G'))
#print(basic_bfs(GRAPH_2, 'S', 'G'))
...
def my_beam_sorting_fn(graph, goalNode, paths, beam_width):
    sorted_paths = sort_paths_by_goal_distance(graph,goalNode,paths)
    level_dic = {}
    sorted_agenda = []
    
    for path in sorted_paths:
        path_level = len(path)
        if not level_dic.get(path_level):
            level_dic[path_level] = 1
            sorted_agenda.insert(0,path)
        elif level_dic[path_level] <  beam_width:
            level_dic[path_level] += 1 
            sorted_agenda.insert(0,path)
...
def is_admissible(graph, goalNode):
    ...
    bb_with_ext = generic_search(*generic_branch_and_bound_with_extended_set)
    graph_copy = graph.copy()
    ideal_paths = [bb_with_ext(graph,n,goalNode) for n in graph.nodes]
    ideal_lengths = [path_length(graph,p) for p in ideal_paths]

    heuristics = [graph.get_heuristic_value(n,goalNode) for n in graph.nodes]
    correct_heuristc = [(h <= l) for h,l in zip(heuristics,ideal_lengths)]
    if(all(correct_heuristc)) : return True
    return False

#test_admissible_graph = GRAPH_FOR_HEURISTICS_TRICKY
#test_admissible_graph.set_heuristic({'G': {'S': 0, 'A': 10, 'B': 5, 'C': 0, 'D': 0, 'G': 0}})
#print(generic_search(*generic_branch_and_bound_with_extended_set)(test_admissible_graph,'A','G'))
...
def is_consistent(graph, goalNode):
    ...
    for node in graph.nodes:
        for neighbor in graph.get_neighbors(node):
            n_heuristic = graph.get_heuristic_value(node, goalNode) 
            neighbor_heuristic = graph.get_heuristic_value(neighbor, goalNode) + graph.get_edge(node,neighbor).length
            if (n_heuristic > neighbor_heuristic): return False
    return True 
...
WHAT_I_FOUND_INTERESTING = "Using a generic search function to create new algorithms "
WHAT_I_FOUND_BORING = "Trying to figure out why test-case #58 fails"
SUGGESTIONS = "I would have loved to implement A* from scratch to really understand the algorithm"
